...
| 版本 | 日期 | 说明 |
|------|------|------|
| 1.4.2 | 2026-08-24 | 访问设置占用检测与三模式兼容完善（按开发文档 22.10 节规范）：设置保存时根路径/路径前缀模式增加占用检测（根路径检测其他已启用应用 home_routes 冲突；路径前缀检测系统保留路径 /admin /user /api /apps、其他应用路由及前缀重合/互为前缀），冲突返回 40002 明细并拒绝落库；新增 `POST /api/admin/niuren/blog/setting/check` 预检接口，设置页切换访问方式或修改前缀时实时展示红色冲突提示区（输入防抖 400ms）；修复子域名模式 404（新增 Routes/home.php 子域名路由入口复用 web.php，系统按域名提前注册并追加 catch-all 404 隔离路由）；前台点赞接口、密码门跳转、详情页返回链接等 4 处硬编码 /blog 路径改为 NR_BASE 动态拼接，评论/点赞/删除/分页/上传等全部前端交互在 root/path/domain 三种访问模式下相互兼容；静态资源版本升至 v1.4.30 并同步 public 副本；新增 NiurenBlogAccessSettingTest 11 用例（占用检测全场景），PostApiTest 26 用例回归通过 |
| 1.4.1 | 2026-08-24 | 前台删除动态修复与交互升级：修复 `DELETE /blog/posts/{id}` 返回值类型声明错误（`JsonResponse` → `array`，此前触发 50001 TypeError）；删除确认由 `window.confirm` 改为 1:1 复刻微信 iOS 风格的居中确认弹窗（半透明黑遮罩 + 白色圆角弹窗，标题 + 说明 + 左「取消」右红色「删除」，缩放淡入动画，PC/移动统一体验，适配深色模式）；PostApiTest 补充前台删除三用例（管理员删除/访客 40301/不存在 40401），共 26 用例 |
